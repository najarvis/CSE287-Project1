...
float normalizeDegrees(float degrees) {
	double magnitude = std::floor(degrees / 360.0f);
	return degrees - 360.0f * magnitude;
}
...
float normalizeRadians(float rads) {
	double magnitude = std::floor(rads / M_2PI);
	return rads - M_2PI * magnitude;
}
...
float directionInDegrees(float x1, float y1, float x2, float y2) {
	return glm::mod(glm::degrees(glm::atan(y2 - y1, x2 - x1)), 360.0f);
}
...
float directionInDegrees(const glm::vec2 &pt1, const glm::vec2 &pt2) {
	return glm::mod(glm::degrees(glm::atan(pt2.y - pt1.y, pt2.x - pt1.x)), 360.0f);
}
...
float directionInDegrees(const glm::vec2 &targetPt) {
	return glm::mod(glm::degrees(glm::atan(targetPt.y, targetPt.x)), 360.0f);
}
...
bool isOrthogonal(const glm::vec3 &a, const glm::vec3 &b) {
	return glm::dot(a, b) == 0 || approximatelyZero(glm::dot(a, b));
}
...
glm::vec3 project(const glm::vec3 &from, const glm::vec3 &onto) {
	// proj(from, to) = (dot(from, to) / (length(to) ^ 2)) * to;
	float l = glm::length(onto);
	return (glm::dot(from, onto) / (l * l)) * onto;
}
...
std::vector<float> quadratic(float A, float B, float C) {
	...
	// Get the first root
	float x_1 = (-1.0f * B + sqrt(inside)) / (2.0f * A);

	// if inside is 0, the roots would be the same
	if (inside != 0.0f) {
		float x_2 = (-1.0f * B - sqrt(inside)) / (2.0f * A);
...
int quadratic(float A, float B, float C, float roots[2]) {
	std::vector<float> found_roots = quadratic(A, B, C);

	for (int i = 0; i < found_roots.size(); i++) {
		roots[i] = found_roots.at(i);
	}
	return found_roots.size();
}
...
float areaOfParallelogram(const glm::vec3 &v1, const glm::vec3 &v2) {
	// area is just the magnitude of the cross product
...
glm::vec3 normalFrom3Points(const glm::vec3 &pt0, const glm::vec3 &pt1, const glm::vec3 &pt2) {
	return myNormalize(glm::cross(pt1 - pt0, pt2 - pt0));
}
...
glm::vec3 normalFrom3Points(const std::vector<glm::vec3> pts) {
	return normalFrom3Points(pts.at(0), pts.at(1), pts.at(2));
}
...
glm::vec3 solveLinearSystem(const glm::mat3 &M, const glm::vec3 &y) {
	if (!isInvertible(M)) return glm::vec3(0.0f, 0.0f, 0.0f);
...
std::vector<glm::vec3> multiplyMatricesAndVertices(const std::vector<glm::mat3> &M, const std::vector<glm::vec3> &verts) {
	...
	return multiplyMatrixAndVertices(BIGMAT, verts);
}
